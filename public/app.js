  ...
  paypal.HostedFields.render({
    ...
    document.querySelector("#card-form").addEventListener("submit", (event) => {
      ...
      cardFields
        ...
        .then((payload) => {
          console.log(
            `3DS-CONTINGENCY: liabilityShifted: ${
              payload.liabilityShifted
            } | authenticationStatus: ${
              payload.authenticationStatus
            } | authenticationReason: ${
              payload.authenticationReason
            } | Payload: ${JSON.stringify(payload)}`
          );
          var redirectURL = "review?token=" + orderId;
          var threeDSLiabilityShifted = "";
          var threeDSAuthenticationStatus =
            "&3ds_auth_status=" + payload.authenticationStatus;
          var threeDSAuthenticationReason =
            "&3ds_auth_reason=" + payload.authenticationReason;
          redirectURL =
            redirectURL +
          ...
          href =
            "https://www.sandbox.paypal.com/webapps/helios?action=verify&flow=3ds&cart_id=" +
            orderId +
            "&redirect_uri=" +
            redirect;
          //        document.getElementById('threeds').innerHTML="<Iframe src="+href+" />";
          forthreeds =
            "<Dialog open> <a href='#' onclick=captureOrder('" +
            orderId +
            '\')>Close</a> <Iframe height="500" width="500" src=' +
            href +
            ' sandbox="allow-scripts allow-popups allow-same-origin" referrerpolicy="origin" /> </Dialog>';
